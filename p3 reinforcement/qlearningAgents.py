...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        action = None
        "*** YOUR CODE HERE ***"
        act_randomly = util.flipCoin(self.epsilon)
        if act_randomly:
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        sample_ = reward + self.discount * self.computeValueFromQValues(nextState)
        self.q_values[(state, action)] = (1 - self.alpha) * self.q_values[(state, action)] + self.alpha * sample_
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            ...
            print("Discount rate(discount) : {0}".format(self.discount))
