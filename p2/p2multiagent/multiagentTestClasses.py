...
#     state.isWin()
#     state.isLose()
#     state.getNextState(agentIndex, action)
#     state.getScore()
...
class MultiagentTreeState(object):
    ...
    def getNextState(self, agentIndex, action):
        if VERBOSE:
            print("getNextState(%s, %s, %s) -> %s" % (self.state, agentIndex,
                                                           action, self.problem.stateToChildMap[self.state][action]))
        child = self.problem.stateToChildMap[self.state][action]
        self.problem.generatedStates.add(child)
        return MultiagentTreeState(self.problem, child)
...
class MultiagentTreeProblem(object):
    def __init__(self, numAgents, startState, winStates, loseStates, children, evaluation):
        ...
        self.stateToChildMap = defaultdict(dict)
        self.stateToActions = defaultdict(list)
        for state, action, nextState in children:
            self.stateToActions[state].append(action)
            self.stateToChildMap[state][action] = nextState
...
def parseTreeProblem(testDict):
    ...
    for line in testDict["children"].split('\n'):
        tokens = line.split()
        if len(tokens) == 3:
            state, action, nextState = tokens
            children.append((state, action, nextState))
        else:
            raise Exception("[parseTree] Bad children line: |%s|" % (line,))

    return MultiagentTreeProblem(numAgents, startState, winStates, loseStates, children, evaluation)
